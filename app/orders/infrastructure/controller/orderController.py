...
@router.patch("/orders/update/{order_id}", status_code=status.HTTP_200_OK, dependencies=[Depends(RoleChecker(["manager"]))], response_model=None)
async def update_completed_order(current_user: Annotated[User, Depends(get_current_user)], order_id: str, order_update: UpdateOrderDTO, session: AsyncSession = Depends(database.get_session)):
    repo = OrderRepository(session)
    repoI = InventoryRepository(session)
    inventory_service = GetInventoryByProductIdService(repoI)
    order_service = UpdateOrderStateByIdService(repo, inventory_service)
    try:
...
async def update_canceled_order(current_user: Annotated[User, Depends(get_current_user)], order_id: str, order_update: UpdateOrderDTO, session: AsyncSession = Depends(database.get_session)):
    repo = OrderRepository(session)
